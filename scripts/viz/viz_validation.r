...
args <- c('results/validation_results.tsv', 'data/protein_quantification.csv', 'figures/validation_')
print(args)
...
qc_pass <- c('CB2', 'H5M', '3HQ', 'PBH', 'LNA', 'YXL', 'WN_', 'R7K', 'RJW', 'TRM', 'UNY', 'W31')
...
# Plot Bar Chart
###################################
png(figure_box, height = 370, width = 550)
ggplot(validation, aes(x = sample_id, y = weighted_auc, fill = sample_id)) +
  geom_boxplot() + xlab('') + ylab('Weighted AUROC') +
  geom_hline(yintercept = 0, linetype="dashed", lwd = 1.5) +
...
# Pearson correlation
results <- cor.test(x = cor_test_ready$weight_auc, y = cor_test_ready$u87pi.norm, method = 'spearman')

# Plot
add_text <- paste0('rho = ', round(results$estimate, 2) , '\np = ', round(results$p.value, 2))
png(figure_scatter, height = 370, width = 450)
ggplot(plot_ready, aes(x = weight_auc, y = u87pi.norm, color = plate, label = sample_id)) +
  xlab('Weighted Predictions (Mean)') + ylab('U87PI Normalized Protein') +
  geom_point() + geom_text(color = 'black', vjust = -0.3) +
  ...
  annotate('text', x = 0.4, y = 1.4, label = add_text, size = rel(6))
dev.off()
...
# Refactor for plotting
predict_plot$predict_nf1 <- factor(predict_plot$predict_nf1, levels = c('NF1 Wildtype', 'NF1 Inactive'))
