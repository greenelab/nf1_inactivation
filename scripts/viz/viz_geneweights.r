...
ggplot(weight, aes(x = rank, y = weight)) +
  ...
  geom_hline(yintercept = high_weight, linetype = "dashed", lwd = 1.2,
             color = "red") +
  geom_hline(yintercept = -high_weight, linetype = "dashed", lwd = 1.2,
             color = "red") +
...
high_weight_pos <- high_weight_pos[order(high_weight_pos$weight,
                                         decreasing = T), ]
...
write.table(high_weight_pos, "results/high_weight_genes_positive.tsv",
            sep = "\t", row.names = F)
write.table(high_weight_neg, "results/high_weight_genes_negative.tsv",
            sep = "\t", row.names = F)
